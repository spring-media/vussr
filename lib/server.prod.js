...
const serveApp = require('./initialize-app');
...
class ProdServer {
  ...
  constructor(options) {
    ...
    serveApp(this.express, this.config, this.getRenderFunction());
}
  ...
  listenOnExpress(port) {
    return new Promise((res, rej) => { const listener = this.express.listen(port, err => err ? rej(err) : res(listener)) })
  }

...
class DevServer {
  constructor(options) {
    ...
    this.devServer = new WebpackDevServer(this.compiler, { ...this.config.devServer, after: app => this.after(app) });
  ...
  async listen() {
    ...
    this.listener = await listenAsPromised(this.devServer, this.devServer, this.port, this.host);

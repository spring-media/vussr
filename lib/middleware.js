...
function getHeaderAgeClass(res, req){
  // get X-AgeClass Header based on req.data.article.published_timestamp
  console.log('getHeaderAgeClass: ' + req.data.article);
  return 'G';
}
...
function getContext(req) {
  ...
  function addDataToRequest(key, data){
    req.data[key] = data;
  }

  return { url, fullUrl, mobile, desktop, addDataToRequest };
}
...
module.exports = (render, errorHandler = defaultErrorHandler) => async (req, res, next) => {
  try {
    ...
    console.log(req.data.article);
    res.set({
      'Vary': 'X-Forwarded-Proto, Accept-Encoding',
      'X-AgeClass': getHeaderAgeClass(res, req),
